...
struct Object3D {
    ...
    // Variáveis para controle da trajetória
    std::vector<glm::vec3> controlPoints; // Lista de pontos de controle espaciais
    int currentTargetIndex;               // Índice do ponto atual que o objeto está buscando
    bool isMovingOnTrajectory;            // Controle para ligar/desligar animação
    ...
    Object3D(GLuint vao, int nVerts, glm::vec3 pos, string texName, GLuint texID, glm::vec3 ka, glm::vec3 kd, glm::vec3 ks, float ns) {
        ...
        //Inicializa as variáveis da trajetória
        currentTargetIndex = 0;
        isMovingOnTrajectory = false;
    }
};
...
int main()
{
    ...
    GLuint vao0 = setupGeometry("../assets/Modelos3D/Vozhd.obj", nVerticesObj0, texObj0, ka0, kd0, ks0, ns0);
    ...
    cout << "[P] - Adiciona a posicao atual do objeto a sua trajetoria" << endl;
    cout << "[ESPACO] - Inicia/Pausa a movimentacao da trajetoria" << endl;
    cout << "[C] - Limpa todos os pontos de trajetoria do objeto" << endl;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Lógica de atualização da trajetória (Interpolação linear cíclica)
        for (size_t i = 0; i < cena.size(); i++) {
            Object3D& trajObj = cena[i];
            
            // Verifica se a trajetória está ativa e se possui no mínimo 2 pontos para navegar
            if (trajObj.isMovingOnTrajectory && trajObj.controlPoints.size() > 1) {
                glm::vec3 targetPos = trajObj.controlPoints[trajObj.currentTargetIndex];
                glm::vec3 direction = targetPos - trajObj.position;
                float distance = glm::length(direction);
                
                float moveSpeed = 1.5f; // Velocidade da translação pela trajetória
                float step = moveSpeed * deltaTime;

                if (distance <= step) {
                    // Se o objeto chegou no ponto ou vai ultrapassar ele, muda a posição para o ponto alvo
                    trajObj.position = targetPos;
                    // Avança para o próximo ponto de forma cíclica
                    trajObj.currentTargetIndex = (trajObj.currentTargetIndex + 1) % trajObj.controlPoints.size();
                } else {
                    // Senão, continua movendo em direção ao alvo linearmente
                    trajObj.position += glm::normalize(direction) * step;
                }
            }
        }
        ...
        lightPositions[0] = glm::vec3(4.0f , 3.0f , 4.0f );
        lightColors[0] = glm::vec3(1.0f, 1.0f, 1.0f); // 100% intensidade
        
        // 2. Fill Light (Preenchimento): Menos intensa, lado oposto da Key Light para suavizar sombra (-X, +Y, +Z)
        lightPositions[1] = glm::vec3(-4.0f, 1.5f, 3.0f );
        ...
        lightPositions[2] = glm::vec3(0.0f, 3.0f, -4.0f);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    // Controle de trajetória e prints para auxiliar visualização
    if (action == GLFW_PRESS) {
        // [P] - Salva o ponto atual
        if (key == GLFW_KEY_P) {
            obj.controlPoints.push_back(obj.position);
            cout << ">>> Ponto salvo na trajetoria do Objeto " << objSelecionado << "! Total de pontos: " << obj.controlPoints.size() << endl;
        }
        // [ESPAÇO] - Inicia ou pausa a movimentação
        if (key == GLFW_KEY_SPACE) {
            if (obj.controlPoints.size() > 1) {
                obj.isMovingOnTrajectory = !obj.isMovingOnTrajectory;
                cout << ">>> Trajetoria do Objeto " << objSelecionado << (obj.isMovingOnTrajectory ? " INICIADA" : " PAUSADA") << endl;
            } else {
                cout << ">>> Adicione pelo menos 2 pontos (Tecla P) para iniciar a trajetoria!" << endl;
            }
        }
        // [C] - Clear, limpa a trajetória atual do objeto
        if (key == GLFW_KEY_C) {
            obj.controlPoints.clear();
            obj.currentTargetIndex = 0;
            obj.isMovingOnTrajectory = false;
            cout << ">>> Trajetoria do Objeto " << objSelecionado << " redefinida!" << endl;
        }
    }
